...
test('يتنقل عبر الشاشات التجريبية ويحدد إجابة', async ({ page }) => {
  ...
  const steps = page.getByRole('navigation', { name: 'خطوات الجولة التدريبية' });
  await expect(page.getByText('لا توجد غرفة أو قائمة لاعبين')).toBeVisible();
  ...
  await page
    .getByRole('navigation', { name: 'خطوات الجولة التدريبية' })
    .getByRole('link', { name: 'النتائج' })
    .click();
  await expect(page.getByRole('heading', { name: 'الجزائر' })).toBeVisible();
  await page
    .getByRole('navigation', { name: 'خطوات الجولة التدريبية' })
    .getByRole('link', { name: 'الملخص' })
    .click();
  await expect(page.getByRole('heading', { name: 'ملخص جولتك' })).toBeVisible();
});
