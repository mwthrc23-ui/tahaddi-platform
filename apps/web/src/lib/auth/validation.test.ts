...
import { recoverySchema, signInSchema, signUpSchema } from './validation';
...
describe('auth validation', () => {
  ...
  it('requires stronger passwords for registration', () => {
    expect(signUpSchema.safeParse({ name: 'سارة', email: 'sara@example.com', password: 'short' }).success).toBe(false);
    expect(signUpSchema.safeParse({ name: 'سارة', email: 'sara@example.com', password: 'StrongPass123' }).success).toBe(true);
  });
