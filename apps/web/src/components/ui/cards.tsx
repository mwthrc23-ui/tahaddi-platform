...
type ShowcaseCardProps = { title: string; description?: string; meta?: string; icon?: ReactNode; className?: string };
export function GameCard({ title, description, meta, icon = <Gamepad2 /> }: ShowcaseCardProps) { return <Card className="showcase-card"><span className="card-icon">{icon}</span><h3>{title}</h3><p>{description}</p>{meta && <Badge>{meta}</Badge>}</Card>; }
export function CompetitionCard({ title, description, meta }: ShowcaseCardProps) { return <Card className="showcase-card competition-card"><span className="card-icon"><Trophy /></span><Badge className="badge-live">مباشر</Badge><h3>{title}</h3><p>{description}</p><span className="card-meta"><Users size={16} />{meta}</span></Card>; }
