...
منصة عربية للمسابقات والألعاب والأسئلة المباشرة. يضم المستودع البنية التقنية ونظام تصميم عربي RTL، مع مصادقة، بنك أسئلة، منشئ مسابقات، ومسار MVP أولي للجلسات المباشرة المحفوظة في PostgreSQL.
...
صفحة مرجع الواجهة متاحة في `/design-system`، والشاشات التجريبية في `/demo/waiting` و`/demo/question` و`/demo/results` و`/demo/winners`. يبدأ مسار الجلسة المباشرة من `/host` بعد تسجيل الدخول، وينضم اللاعب من نموذج الرمز في الصفحة الرئيسية إلى `/live/[sessionId]/play`.
...
- [ ] البث اللحظي الكامل عبر Socket.IO، تعدد الأسئلة الخاضع للمضيف، التقارير، والإدارة.
